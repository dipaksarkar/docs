...
## Understanding Mailzor Scan Speeds

From our testing, Mailzor's email verification process with a 50 batch size setting typically validates 1-3 emails per second in a single request. This translates to approximately 150,000 email validations per day per instance. 

However, it's important to understand that verification speed depends heavily on server-to-server socket connections. While your server will respond quickly when you initiate verification, if the target email server delays its connection to your server, this will slow down the entire scan process. This is something neither you nor Mailzor has control over – the system must wait for the target server's final response to complete verification.

## Common Factors Affecting Scan Speed

### Mixed Email Quality

When users upload mixed email lists containing some addresses with complete connection failures or bad response data, it isn't Mailzor's fault but rather an issue with the target server. For these problematic email addresses, Mailzor attempts to wait until the default connection timeout of 10 seconds has passed.

To improve efficiency, consider adding known problematic domains to your disposable email list in the settings panel so Mailzor can skip them immediately. While Mailzor comes with a pre-configured disposable email list, it doesn't contain all disposable addresses worldwide.
...
Consider using a different VPS provider or server location that might have better connectivity to the email servers you're targeting. See our [VPS Recommendations](/mailzor/faqs/vps-recommendations) for suggestions.
...
- Delist your IP from DNSBL services if it's blacklisted (see [Why IPs Get Blacklisted](/mailzor/faqs/why-ip-get-blacklisted))
...
### 6. Use Proxies Strategically

If your server IP is throttled or has poor reputation, consider using a high-quality SOCKS5 proxy. See our [Proxy Guide](/mailzor/proxies) for more information.
...
- Split your email lists among several Mailzor instances
- Use different IP addresses to avoid rate limiting
- Implement [IP warming](/mailzor/faqs/what-is-ip-warming) for each new server
