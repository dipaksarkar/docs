...
# Server Requirements for Validora SaaS

Validora is a Software as a Service (SaaS) application that requires specific server environments to run efficiently. This guide outlines the recommended server infrastructure and environment configurations for optimal performance.

## Minimum Server Requirements

As a SaaS application, Validora needs robust infrastructure to handle multiple users, concurrent verification processes, and data storage. Here are the minimum server requirements:
...
- **PHP Requirements**:
  ...
  - Required extensions as listed in the [Installation guide](/validora/installation)

### Cloud Provider Recommendations

Validora performs well on major cloud platforms, including:
...
For optimal performance of Validora, we recommend using reliable VPS providers that support the requirements listed above. Not all VPS providers offer suitable environments for email verification services, particularly regarding SMTP port 25 accessibility.

For detailed recommendations on specific VPS providers and control panels that work well with Validora, please see our [VPS Recommendations Guide](/validora/faqs/vps-recommendations).
...
Since Validora involves email verification, proper email infrastructure is critical:
...
- **IP Warming**: Follow proper [IP warming procedures](/validora/faqs/what-is-ip-warming)
...
Validora supports containerized deployment using Docker:

```yaml
version: "3"
services:
  app:
    image: validora/app:latest
    ports:
      - "80:80"
    environment:
      - DB_HOST=db
...
Running Validora as a SaaS application requires proper server infrastructure planning. By following these recommendations, you can ensure your Validora installation performs optimally, scales effectively, and provides reliable service to your users.
